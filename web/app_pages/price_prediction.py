...
@st.cache_resource
def load_model():
    with open('model_catalunya.pkl', 'rb') as f:
        model = pickle.load(f)
    return model
...
def load_label_encoders():
    ...
    return le_location, le_type, le_region
# -- -- #


def app():
    data = load_data()
    data = data.drop(columns=['price/m2']) # Eliminem la columna 'price/m2' perquè no la utilitzem en la predicció
    ...
    with tab1:
        ...
        col1, col2, col3, col4, col5, col6 = st.columns(6) # TODO: Ens quedem amb "location" o amb "region"??
        with col1:
            st.subheader('Title')
        ...
        # -- Predicción -- #
        #TODO: Botón + cargar modelo y label encoders + predecir
        #model = load_model()
        #le_location, le_type, le_region = load_label_encoders()
        ...
        if st.button('Predecir precio'):
            st.write('Precio predicho: xxx.xxx €')
        # -- -- #
